...
ScheduleY1 <- read.csv("ScheduleY1.csv")

ScheduleY1 %>% 
  dplyr::select(Week, Lecture, Lab, Reading, DS) %>% 
  # mutate(
  #     slideslink = map(slideslink, ~ htmltools::a(href = .x, "slides")),
  #     slideslink = map(slideslink, ~ gt::html(as.character(.x)))) %>%
  # dplyr::mutate(man = ifelse(slideslink != "null", "gear", "gears")) %>%
  gt() %>%
  # gt_fa_column(man) %>% 
  gt_theme_538()
